...
import { parseGenerateScoreResult, parseRegenerateRegionResult } from '@sudobility/music_types';
...
export class ApiGenerationProvider implements MusicGenerationProvider {
  ...
  async generateScore(
    ...
    const result = await this.context.client.generateScore(request, token, signal);
  ...
  async regenerateRegion(
    ...
    const token = await requireToken(this.context);
    const result = await this.context.client.regenerateRegion(request, token, signal);
