 ...
 */
export function selectionToRange(score: Score, sel: ScoreSelection): ScoreRange | null {
  ...
  const tracksInScope: Track[] =
    scopedTrackIds.length > 0
      ? scopedTrackIds.map((id) => findTrack(score, id)).filter((t): t is Track => t !== null)
      : score.tracks;
  ...
  return { startTick: alignedStart, endTick: alignedEnd, trackIds: scopedTrackIds };
}
 ...
 */
export function normalizeSelection(score: Score, sel: ScoreSelection): ScoreSelection {
  ...
  if (sel.range) {
    ...
    normalized.range = { startTick, endTick, trackIds: dedupe(sel.range.trackIds) };
  }
 ...
 */
export function selectionIsRegenerable(score: Score, sel: ScoreSelection): boolean {
