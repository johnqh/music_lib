...
describe('usablePageHeight', () => {
  it('is the logical width scaled by the printable aspect ratio', () => {
    ...
    expect(usablePageHeight('a4', 'portrait', 1000)).toBeCloseTo(
      (1000 * 273) / 186,
      4
    );
  });
  ...
  it('swaps the dimensions for landscape', () => {
    expect(usablePageHeight('a4', 'landscape', 1000)).toBeCloseTo(
      (1000 * 186) / 273,
      4
    );
  });
  ...
  it('gives legal more height than letter, which is the same width', () => {
    expect(PAPER_DIMENSIONS_MM.legal.width).toBe(
      PAPER_DIMENSIONS_MM.letter.width
    );
...
const flat = (pages: { systemIndices: number[] }[]) =>
  pages.flatMap(p => p.systemIndices);
...
describe('paginate', () => {
  ...
  it('never exceeds the page height unless one system alone cannot fit', () => {
    ...
    for (const page of pages) {
      ...
      if (page.systemIndices.length > 1)
        expect(used).toBeLessThanOrEqual(pageHeight);
    }
  });
  ...
  it('puts everything on one page when it all fits', () => {
    expect(
      paginate(planOf([50, 50, 50]), 1000).map(p => p.systemIndices)
    ).toEqual([[0, 1, 2]]);
  });
});
...
describe('turnFreeBars', () => {
  ...
  it('is zero when the player is playing on both sides of the turn', () => {
    ...
    expect(
      turnFreeBars(plan, trackOf(['note', 'note', 'note', 'note']), 0)
    ).toBe(0);
  });
  ...
  it('is zero after the last system, where there is no turn', () => {
    ...
    expect(turnFreeBars(plan, trackOf(['note', null, null, 'note']), 1)).toBe(
      0
    );
  });
});
...
describe('paginate with a turn track', () => {
  it('pulls the break back onto a rest', () => {
    ...
    expect(paginate(plan, 350).map(p => p.systemIndices)).toEqual([
      [0, 1, 2],
      [3],
    ]);
  ...
  it('never pulls back more than MAX_PULL_BACK systems', () => {
    ...
    expect(paginate(plan, 550, track)[0].systemIndices).toEqual([
      0, 1, 2, 3, 4,
    ]);
  ...
  it('never pulls back to an empty page', () => {
    ...
    expect(paginate(plan, 300, track).map(p => p.systemIndices)).toEqual([
      [0],
      [1],
    ]);
  });
  ...
  it('leaves a whole-score print packed greedily', () => {
    ...
    expect(paginate(plan, 350, undefined).map(p => p.systemIndices)).toEqual(
      greedy
    );
  });
...
describe('paginate on a real score', () => {
  ...
  it('never turns worse than greedy would, and turns better somewhere', () => {
    ...
    const scoreOf = (breaks: number[]) =>
      breaks.map(i => turnFreeBars(plan, track, i));
    ...
    expect(
      tunedScores.slice(0, shared).some((s, i) => s > greedyScores[i])
    ).toBe(true);
  });
