...
async function importedOnsetSeconds(
  beats: number[],
  lengthBeats: number
): Promise<number[]> {
  const midi = new Midi();
  ...
  const notes = score.tracks[0].measures
    ...
    .filter(
      (event): event is typeof event & { startTick: number } => 'pitch' in event
    );
...
async function expectPreserved(
  ...
  beats.forEach((beat, i) => {
    expect(Math.abs(imported[i] - beat * SECONDS_PER_BEAT)).toBeLessThan(
      TOLERANCE_SECONDS
    );
  });
}
...
describe('midi import timing', () => {
  ...
  it('keeps sixteenths and triplets together in one file', async () => {
    await expectPreserved(
      [0, 0.25, 0.5, 0.75, 1, 4 / 3, 5 / 3, 2, 2.25, 2.5, 2.75, 3],
      0.25
    );
  });
