...
function smf(ppq: number, bpm: number, quarterNotes: number): ArrayBuffer {
  ...
  const u32 = (n: number) => push((n >> 24) & 255, (n >> 16) & 255, (n >> 8) & 255, n & 255);
  const u16 = (n: number) => push((n >> 8) & 255, n & 255);
  push(0x4d, 0x54, 0x68, 0x64); u32(6); u16(0); u16(1); u16(ppq); // MThd, format 0, 1 track
  ...
  tvlq(0); tp(0xff, 0x51, 0x03, (uspq >> 16) & 255, (uspq >> 8) & 255, uspq & 255); // tempo
  for (let i = 0; i < quarterNotes; i++) {
    tvlq(0); tp(0x90, 60, 100);        // note on
    tvlq(ppq); tp(0x80, 60, 0);        // note off one quarter later
  }
  tvlq(0); tp(0xff, 0x2f, 0x00);       // end of track
  ...
  it('places notes at the right wall-clock time whatever the file ppq and tempo', async () => {
    ...
    for (const ppq of [96, 192, 384, 480, 960]) {
      for (const bpm of [60, 90, 120, 144, 180]) {
        ...
        rows.push(`  ppq ${String(ppq).padStart(3)} @ ${String(bpm).padStart(3)}bpm: ${got.length}/8 notes, worst drift ${(max * 1000).toFixed(1)}ms, last note ${got.at(-1)?.toFixed(3)}s (expect ${expected.at(-1)?.toFixed(3)}s)`);
      }
    }
    appendFileSync('/tmp/speed.txt', rows.join('\n') + `\n\nWORST OVERALL: ${(worst * 1000).toFixed(1)}ms\n`);
