...
describe('notateDuration', () => {
  it('notates every base duration as a single, undotted segment', () => {
    expect(notateDuration(ticksFor('whole', PPQ), PPQ)).toEqual([
      { ticks: 1920, type: 'whole', dots: 0 },
    ]);
    expect(notateDuration(ticksFor('half', PPQ), PPQ)).toEqual([
      { ticks: 960, type: 'half', dots: 0 },
    ]);
    expect(notateDuration(ticksFor('quarter', PPQ), PPQ)).toEqual([
      { ticks: 480, type: 'quarter', dots: 0 },
    ]);
    expect(notateDuration(ticksFor('eighth', PPQ), PPQ)).toEqual([
      { ticks: 240, type: 'eighth', dots: 0 },
    ]);
    expect(notateDuration(ticksFor('sixteenth', PPQ), PPQ)).toEqual([
      { ticks: 120, type: '16th', dots: 0 },
    ]);
    expect(notateDuration(ticksFor('thirtysecond', PPQ), PPQ)).toEqual([
      { ticks: 60, type: '32nd', dots: 0 },
    ]);
  });

  it('notates a dotted duration as a single segment with dots: 1', () => {
    expect(notateDuration(ticksFor('dotted-quarter', PPQ), PPQ)).toEqual([
      { ticks: 720, type: 'quarter', dots: 1 },
    ]);
    expect(notateDuration(ticksFor('dotted-half', PPQ), PPQ)).toEqual([
...
describe('ticksForNotatedType', () => {
  it('inverts notateDuration for every base and singly-dotted type', () => {
    ...
    expect(ticksForNotatedType('quarter', 0, PPQ)).toBe(
      ticksFor('quarter', PPQ)
    );
    expect(ticksForNotatedType('16th', 0, PPQ)).toBe(
      ticksFor('sixteenth', PPQ)
    );
    expect(ticksForNotatedType('32nd', 0, PPQ)).toBe(
      ticksFor('thirtysecond', PPQ)
    );
    expect(ticksForNotatedType('quarter', 1, PPQ)).toBe(
      ticksFor('dotted-quarter', PPQ)
    );
    expect(ticksForNotatedType('half', 1, PPQ)).toBe(
      ticksFor('dotted-half', PPQ)
    );
  });

  it('generalizes beyond a single dot (double-dotted = 1.75x base)', () => {
    expect(ticksForNotatedType('quarter', 2, PPQ)).toBe(
      Math.round(ticksFor('quarter', PPQ) * 1.75)
    );
  });
});
