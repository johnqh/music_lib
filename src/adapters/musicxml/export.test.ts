...
function parse(xml: string): Document {
  ...
  expect(parserError.length, `XML failed to parse: ${xml.slice(0, 500)}`).toBe(
    0
  );
  return doc;
}
...
describe('exportMusicXml', () => {
  it('produces a well-formed score-partwise 4.0 document with work title and part-list', () => {
    ...
    expect(textOf(doc.documentElement, 'work > work-title')).toBe(
  ...
  it('emits chord notes with <chord/> on every note after the first at a shared start tick', () => {
    ...
    const firstMeasureNotes = (
      doc.querySelector('part measure') as Element
  ...
  it('splits a duration that does not notate as a single value into tied notes', () => {
    ...
    expect(textOf(doc.documentElement, 'attributes > time > beat-type')).toBe(
      '8'
    );
  ...
  it('emits <tie>/<tied> for a note that already has tieStart/tieStop set (e.g. a tie split at a barline)', () => {
    ...
    expect(
      tieStartNote.querySelector('notations > tied[type="start"]')
    ).not.toBeNull();
    const tieStopNote = measure2Notes[0];
    expect(tieStopNote.querySelector('tie[type="stop"]')).not.toBeNull();
    expect(
      tieStopNote.querySelector('notations > tied[type="stop"]')
    ).not.toBeNull();
  });
  ...
  it('emits articulations mapped to their MusicXML element (marcato -> strong-accent)', () => {
    ...
    const notes = doc.querySelectorAll('part measure note');
    expect(
      notes[0].querySelector('notations > articulations > staccato')
    ).not.toBeNull();
    expect(
      notes[1].querySelector('notations > articulations > accent')
    ).not.toBeNull();
    expect(
      notes[2].querySelector('notations > articulations > tenuto')
    ).not.toBeNull();
    expect(
      notes[3].querySelector('notations > articulations > strong-accent')
    ).not.toBeNull();
  });
  ...
  it('emits a tempo direction with <sound tempo> on the first measure of the first part only', () => {
    ...
    const parts = doc.querySelectorAll('part');
    expect(
      parts[0].querySelector('measure direction sound')?.getAttribute('tempo')
    ).toBe('120');
...
describe('safeFilename', () => {
  it('slugifies a title into a lowercase-hyphenated filename', () => {
    expect(safeFilename('Twinkle Twinkle Little Star')).toBe(
      'twinkle-twinkle-little-star'
    );
  });
